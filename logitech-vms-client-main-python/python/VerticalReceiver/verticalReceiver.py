...
class verticalReceiver(RS485receiver):
    def __init__(self)-> None:
        self.deviceID:str = os.getenv("DEVICE_ID")
        self.deviceType: str = "Vertical Receiver"
        ...
        self.ioConnect()
        super().__init__(port = os.getenv("RS485_PORT_VERTICAL"), 
                        baudrate = os.getenv("RS485_BAUDRATE_VERTICAL"),
                        timeout = int(os.getenv("RS485_TIMEOUT_VERTICAL"))
        )
        return
    ...
    def capture(self, start_time: float) -> None:
        ...
        while self._isCapture:
            ...
            if ret:
                data = dataFrame(res)
                if data.NBB >0:
                    self.Points.append([data.NBB, time()-start_time])
                else: 
                    self.sendSocketData()
                    # print("MLG_H:",result)
    ...
    def sendSocketData(self) -> None:
        self.io.emit("verticalData", self.Points)
        return
    ...
    def run(self):
        ...
        while self._isRunning:
            ...
            if ret:
                self.Points.clear()
                data = dataFrame(res)
                self.Points.append([data.NBB,0])
                self.capture(time())
        self.close()
        
def main():
    test = verticalReceiver()
    test.run()
